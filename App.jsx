...
export default function App() {
  ...
  const [stage,     setStage]      = useState("idle"); // idle|scraping|analysing|done|error
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#07070f", color:"#e0e0f0",
      ...
      {/* NAV */}
      <nav style={{ background:"#0c0c1a", borderBottom:"1px solid #15152a",
      ...
      <div style={{ maxWidth:740, margin:"0 auto", padding:"32px 20px 60px" }}>
        ...
        {/* ── SETUP CARD ──────────────────────────────────────────── */}
        <div style={{ background:"#0e0e1e", border:"1px solid #1a1a2e",
          borderRadius:18, padding:"28px", marginBottom:22 }}>
          ...
          <div style={{ display:"flex", gap:8, marginBottom:6 }}>
            <input type="password" value={apifyToken}
              ...
              style={{ flex:1, background:"#0a0a18",
                border:`1px solid ${tokenSaved ? "#34d399" : "#1e1e30"}`,
                borderRadius:10, padding:"11px 14px", color:"#e0e0f0",
                fontSize:14, fontFamily:"monospace" }} />
            <button className="btn" onClick={() => { if (apifyToken.trim()) setTokenSaved(true); }}
              style={{ padding:"0 18px", borderRadius:10, fontSize:13, fontWeight:700,
                background: apifyToken.trim() ? "linear-gradient(135deg,#6366f1,#a855f7)" : "#1a1a2e",
                color: apifyToken.trim() ? "#fff" : "#444" }}>
              {tokenSaved ? "✅ Saved" : "Save"}
            </button>
          </div>
          <p style={{ fontSize:12, color:"#333", marginBottom:22, lineHeight:1.7 }}>
            Get your free token → <a href="https://apify.com" target="_blank" rel="noreferrer"
              style={{ color:"#6366f1", textDecoration:"none" }}>apify.com</a>
                ...
                style={{ flex:1, padding:"11px", borderRadius:10, fontSize:13, fontWeight:700,
                  background: platform===p.key
                    ? p.key==="instagram"
                      ...
                      : "linear-gradient(135deg,#1d9bf0,#0d8bd9)"
                    : "#111120",
                  color: platform===p.key ? "#fff" : "#555",
          ...
          <input value={postUrl} onChange={e => setPostUrl(e.target.value)}
            ...
            style={{ width:"100%", background:"#0a0a18", border:"1px solid #1e1e30",
              borderRadius:10, padding:"12px 14px", color:"#e0e0f0",
              fontSize:14, marginBottom:6 }} />
          <p style={{ fontSize:12, color:"#333", marginBottom:22 }}>
            Must be a <strong style={{color:"#555"}}>public</strong> post or reel
          </p>
            ...
            {[50,100,250,500].map(n => (
              <button key={n} className="btn" onClick={() => setMaxComments(n)}
                style={{ flex:1, padding:"10px 0", borderRadius:10, fontSize:13, fontWeight:700,
                  background: maxComments===n
                    ? "linear-gradient(135deg,#a78bfa,#6366f1)" : "#111120",
                  color: maxComments===n ? "#fff" : "#555",
                  border:"1px solid #1a1a2e" }}>
                {n}{n===500?" 🔥":""}
              </button>
            ))}
          ...
          {errorMsg && (
            <div style={{ background:"rgba(244,63,94,0.1)", border:"1px solid rgba(244,63,94,0.3)",
              borderRadius:10, padding:"12px 16px", marginBottom:16, fontSize:13, color:"#f43f5e" }}>
              ⚠️ {errorMsg}
            </div>
          )}
          ...
          <button className="btn" onClick={handleRun} disabled={isRunning}
            style={{ width:"100%", padding:"15px", borderRadius:12,
              ...
              background: isRunning ? "#1a1a2e"
                : "linear-gradient(135deg,#6366f1,#ec4899)",
              color: isRunning ? "#444" : "#fff",
              boxShadow: isRunning ? "none" : "0 4px 28px rgba(99,102,241,.4)",
              cursor: isRunning ? "not-allowed" : "pointer" }}>
            {stage==="scraping"  ? "📡 Scraping comments…"
            :stage==="analysing" ? "🤖 Claude is analysing…"
            :stage==="done"      ? "🔄 Analyse Another Post"
        ...
        {/* ── PROGRESS ───────────────────────────────────────────── */}
        {isRunning && (
          <div className="fade" style={{ background:"#0e0e1e", border:"1px solid #1a1a2e",
            borderRadius:16, padding:"28px", textAlign:"center", marginBottom:22 }}>
            <div className="pulse" style={{ fontSize:44, marginBottom:12 }}>
              {stage==="scraping" ? "📡" : "🤖"}
            </div>
            <p style={{ fontWeight:700, fontSize:16, marginBottom:6 }}>
              {stage==="scraping" ? "Scraping comments from "+platform+"…" : "Claude AI is analysing…"}
            </p>
            <p style={{ fontSize:13, color:"#555", marginBottom:20 }}>{stageMsg}</p>
            <div style={{ background:"#1a1a2e", borderRadius:100, height:8,
              overflow:"hidden", maxWidth:380, margin:"0 auto" }}>
              <div className="prog" style={{ height:"100%", borderRadius:100,
                background:"linear-gradient(90deg,#6366f1,#ec4899)", width:`${progress}%` }} />
            </div>
          ...
          <div className="fade">
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:8, marginBottom:18 }}>
              {Object.entries(SEV).map(([key,cfg]) => (
                <div key={key} style={{ background:cfg.bg, border:`1px solid ${cfg.color}25`,
                  borderRadius:12, padding:"14px 8px", textAlign:"center" }}>
                  <div style={{ fontSize:22, marginBottom:4 }}>{cfg.icon}</div>
                  <div style={{ fontSize:22, fontWeight:800, color:cfg.color,
                    fontFamily:"'Space Mono',monospace" }}>{stats?.counts[key]??0}</div>
            ...
            <div style={{ display:"flex", gap:7, flexWrap:"wrap", alignItems:"center", marginBottom:14 }}>
              {[
                ...
                <button key={f.key} className="btn"
                  onClick={() => { setFilter(f.key); setAutoHide(false); }}
                  style={{ padding:"6px 12px", borderRadius:8, fontSize:12, fontWeight:600,
                    background: filter===f.key && !autoHide
                      ? "linear-gradient(135deg,#6366f1,#a855f7)" : "#111120",
                    color: filter===f.key && !autoHide ? "#fff" : "#555",
                    border:"1px solid #1a1a2e" }}>{f.label}</button>
              ))}
              <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:8 }}>
                <span style={{ fontSize:12, color:"#444" }}>Clean only</span>
                <div onClick={() => setAutoHide(v => !v)} style={{ cursor:"pointer",
                  width:38, height:20, borderRadius:100,
                  background:autoHide ? "#6366f1" : "#1a1a2e",
                  position:"relative", transition:"background .2s", flexShrink:0 }}>
                  <div style={{ position:"absolute", top:2, left:autoHide?20:2,
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {visible.length === 0
                ? <div style={{ textAlign:"center", padding:48, color:"#333", fontSize:14 }}>
                        ...
                        <div style={{ display:"flex", gap:12, alignItems:"flex-start" }}>
                          ...
                          <div style={{ flex:1, minWidth:0 }}>
                            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4, flexWrap:"wrap" }}>
                              ...
                              {c.likes > 0 && (
                                <span style={{ fontSize:12, color:"#444", marginLeft:"auto" }}>❤️ {c.likes}</span>
                              )}
                            </div>
                            ...
                            {open && c.reason && (
                              <div style={{ marginTop:10, paddingTop:10,
                                ...
                                <p style={{ fontSize:12, color:"#777" }}>
                                  <span style={{ color:cfg.color, fontWeight:700 }}>AI Reason: </span>
                                  {c.reason}
                                </p>
                              </div>
                            )}
...
function StepLabel({ num, color, text }) {
  return (
    <label style={{ fontSize:12, fontWeight:700, color:"#777", textTransform:"uppercase",
      letterSpacing:"0.6px", display:"flex", alignItems:"center", gap:8, marginBottom:10 }}>
      <span style={{ background:"#1a1a2e", borderRadius:6, padding:"3px 8px", color, fontFamily:"monospace" }}>
        {num}
      </span>
      {text}
    </label>
